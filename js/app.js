(function(){if(window.location.protocol==='file:'){document.body.innerHTML='<h1 style="font-size:28px;font-weight:300;letter-spacing:1px;color:#fff">Run via localhost:8080</h1><p style="font-size:14px;color:#888;max-width:400px;text-align:center;line-height:1.5">Orbit requires a local server. Open http://localhost:8080 in your browser.</p>';throw new Error('file:// execution blocked')}})();
    ...
    // ── Shared Idempotent Homepage Initializer ──
    function initHomepage() {
      ...
      var editIcon = document.querySelector('.hero-edit-icon');
      ...
      if (heroClock) heroClock.style.opacity = '1';

      // Re-render notifications and position below search bar
      var heroNotifs = document.getElementById('heroNotifications');
      if (heroNotifs) {
        notifExpanded = false;
        notifCurrentPage = 0;
      ...
      // Build customization menu content once
      var customMenu = document.getElementById('customizationMenu');
      if (customMenu && !customMenu.hasChildNodes()) {
        buildCustomizationMenu();
      }
      if (customMenu) customMenu.classList.remove('open');

      // Restore saved background before paint
    ...
    function startExperience() {
      initHomepage();
      music.play().catch(() => {});
      var primer = hoverAudio.cloneNode();
      primer.volume = 0;
      primer.play().catch(() => {});
      ...
      // Fade out intro, then remove it and start typing
    ...
    // Particle movement presets — only velocity/direction variables, no recreation
    function applyParticleMotion(mode) {
      particleMode = mode || 'float-up';
      particles.forEach(function(p) {
        // Store base speed multiplier from random variance
        var speedBase = Math.abs(p._speedVx || (Math.random() - 0.5) * 0.2);
        p._speedVx = speedBase;
        p._speedBaseY = Math.abs(p._speedBaseY || Math.random() * 0.2 + 0.1);
        switch (particleMode) {
          case 'float-up':
            p.vx = (Math.random() - 0.5) * speedBase * 0.5;
            p.vy = -p._speedBaseY;
            p.wobbleAmplitude = Math.random() * 0.08 + 0.02;
            break;
          case 'float-down':
            p.vx = (Math.random() - 0.5) * speedBase * 0.5;
            p.vy = p._speedBaseY;
            ...
            break;
          case 'left-to-right':
            p.vx = p._speedBaseY * 0.8;
            p.vy = (Math.random() - 0.5) * speedBase * 0.3;
            p.wobbleAmplitude = Math.random() * 0.04 + 0.01;
            break;
          case 'right-to-left':
            p.vx = -p._speedBaseY * 0.8;
            p.vy = (Math.random() - 0.5) * speedBase * 0.3;
          ...
          case 'bottom-left-top-right':
            p.vx = p._speedBaseY * 0.6;
            p.vy = -p._speedBaseY * 0.6;
            p.wobbleAmplitude = Math.random() * 0.05 + 0.01;
            break;
          ...
          case 'top-right-bottom-left':
            ...
            break;
          case 'gentle-wave':
            p.vx = (Math.random() - 0.5) * speedBase * 0.3;
            p.vy = -(Math.random() * 0.1 + 0.05);
            p.wobbleAmplitude = Math.random() * 0.2 + 0.1;
            break;
          case 'slow-drift':
            p.vx = (Math.random() - 0.5) * speedBase * 0.2;
            p.vy = -(Math.random() * 0.05 + 0.02);
            p.wobbleAmplitude = Math.random() * 0.03 + 0.01;
            break;
          default:
            p.vx = (Math.random() - 0.5) * speedBase * 0.5;
            p.vy = -p._speedBaseY;
        ...
        // Reset wobble phase for smooth transition
        p.wobble = Math.random() * Math.PI * 2;
      });
      localStorage.setItem('orbit-particle-mode', particleMode);
    }

    class Particle {
      constructor(initial = false) {
        this._speedVx = (Math.random() - 0.5) * 0.2;
        this._speedBaseY = Math.random() * 0.2 + 0.1;
        this.wobbleAmplitude = 0.08;
      ...
      update(time) {
        ...
        this.x += this.vx + Math.sin(this.wobble) * this.wobbleAmplitude;
        this.y += this.vy;
    ...
    // Apply saved particle mode after creation
    (function initParticleMode() {
      var savedMode = localStorage.getItem('orbit-particle-mode');
      if (savedMode) applyParticleMotion(savedMode);
      else applyParticleMotion('float-up');
    })();
    // Apply saved background before first paint to avoid flash
    (function applyBgBeforePaint() {
      var mode = localStorage.getItem('orbit-bg-mode');
      var bg = localStorage.getItem('orbit-bg');
      if (mode === 'image' && bg) {
        var root = document.documentElement;
        root.style.setProperty('--custom-bg', 'url(backgrounds/' + bg + ')');
    ...
    const cookieUrl = "https://outred.org/g/assets/cookie-clicker/index.html";
    ...
    const basketballStarsUrl = "https://outred.org/g/assets/basketball-stars/index.html";
    const clusterRushIcon = "https://outred.org/g/assets/cluster-rush/splash.png";
    const clusterRushUrl = "https://outred.org/g/assets/cluster-rush/index.html";
    const driftHuntersIcon = "https://outred.org/g/assets/drift-hunters/drift-hunters.png";
    const driftHuntersUrl = "https://outred.org/g/assets/drift-hunters/index.html";
    ...
    const elasticManUrl = "https://outred.org/g/assets/elasticman/index.html";
    ...
    const fireboyWatergirlUrl = "https://incrediblewebsite.github.io/fbwg2/";
    ...
    const sectionData = {
      games: [
        ...
        },
        {
          ...
          url: "https://sussygamedeveloper.github.io/FNAF1/",
          source: "github-pages",
        },
        {
          id: "fnaf-2",
          ...
          url: "https://ruihq.github.io/FNAF2/index.html",
          source: "github-pages",
        },
        {
          id: "fnaf-3",
          ...
          url: "https://irv77.github.io/hd_fnaf/3/",
          source: "github-pages",
        },
        {
          id: "fnaf-4",
        ...
        },
        {
          ...
          url: "https://bitent.com/swf/papas_burgeria/",
          source: "external",
        },
        {
          id: "papas-pizzeria",
        ...
        },
        {
          ...
          url: "https://driftbossonline.github.io/",
          source: "github-pages",
        },
        {
          id: "mario-kart-64",
          ...
          url: "https://sharkdevelopmentinc.github.io/mariokart64/dist/",
          source: "github-pages",
        },
        {
          id: "duck-life",
        ...
        },
        {
          ...
          url: "https://outred.org/g/assets/ducklife2/index.html",
          source: "self-hosted",
        },
        {
          id: "duck-life-3",
          ...
          url: "https://outred.org/g/assets/ducklife3/index.html",
          source: "self-hosted",
        },
        {
          id: "duck-life-4",
          ...
          url: "https://outred.org/g/assets/ducklife4/index.html",
          source: "self-hosted",
        },
        {
          id: "cookie-clicker",
        ...
        },
        {
          ...
          url: "https://supermario-game.github.io/",
          source: "github-pages",
        },
        {
          id: "super-mario-64",
          ...
          url: "https://outred.org/g/assets/sm64/index.html",
          source: "self-hosted",
        },
        {
          id: "elastic-man",
        ...
        },
        {
          ...
          url: "https://outred.org/g/assets/motox3m/index.html",
          source: "self-hosted",
        },
        {
          id: "retro-bowl",
        ...
        },
        {
          ...
          url: "https://outred.org/g/assets/ovo/index.html",
          source: "self-hosted",
        },
        {
          id: "temple-run-2",
        ...
        },
        {
          ...
          url: "https://outred.org/g/assets/rooftop-snipers/index.html",
          source: "self-hosted",
        },
        {
          ...
          url: "https://incrediblewebsite.github.io/fbwg2/",
          source: "github-pages",
        },
        {
          ...
          url: "", // funkypotato.com sends frame-ancestors CSP blocking iframe embed on other domains
          source: "needs-mirror",
        },
        {
          id: "paper-io-2",
          ...
          url: "https://outred.org/g/assets/paperio2/index.html",
          source: "self-hosted",
        },
        {
          ...
          url: "https://plantsvszombiesjs.github.io/",
          source: "github-pages",
        },
        {
          id: "snowball-io",
          ...
          url: "https://snowballio.gitlab.io/",
          source: "gitlab-pages",
        },
        {
          id: "lolbeans",
        ...
        },
        {
          ...
          url: "https://ubg69.gitlab.io/game/buckshot-roulette.html",
          source: "gitlab-pages",
        },
        {
          id: "burger-frights",
        ...
        },
        {
          ...
          url: "https://menkoi.github.io/Backrooms/",
          source: "github-pages",
        },
        {
          id: "granny",
        ...
        },
        {
          ...
          url: "https://happy-wheels-online.github.io/",
          source: "github-pages",
        },
        {
          id: "slow-roads",
        ...
        },
        {
          ...
          url: "", // NEEDS MIRROR — no clean playable source currently available
          source: "needs-mirror",
        },
        {
          id: "half-life",
        ...
        },
        {
          ...
          url: "https://outred.org/g/assets/sort-the-court/index.html",
          source: "self-hosted",
        },
        {
          id: "doom-2",
    ...
    const cyclingPhrases = [
      ...
      "Use the sidebar to switch between Games, Proxies, and Tools.",
...
<body>
  <iframe src="${src}" title="${safeTitle}" allow="fullscreen" allowfullscreen loading="lazy"></iframe>
    ...
    const fullPageSections = ['settings', 'games', 'search', 'tools', 'apps', 'browser'];
    ...
    function buildThumb(item) {
      if (item.image) {
        return `<img src="${escapeHTML(item.image)}" alt="${escapeHTML(item.title)} icon" data-emoji="${escapeHTML(item.emoji)}">`;
      }
          ...
          <div class="game-notch" id="gameNotch">
            <div class="game-notch-left">
              <div class="game-notch-icon">
                <img src="${escapeHTML(game.image)}" alt="${escapeHTML(game.title)} icon" onerror="this.onerror=null;this.outerHTML='<span style=display:flex;align-items:center;justify-content:center;width:100%;height:100%;font-size:1.5rem>${escapeHTML(game.emoji)}</span>'">
              </div>
    ...
    function render(section, query = '') {
      ...
      // Show loading spinner on first games page load (non-blocking)
      if (section === 'games' && firstGamesPageLoad) {
        ...
        // Hide game cards behind the overlay until their images are ready
        mainContent.style.visibility = 'hidden';
        // Fallback timeout: reveal content after 8s regardless of image load status
        window.__gamesLoadTimeout = setTimeout(function() {
          var overlay = document.getElementById('gamesPageLoadingOverlay');
      ...
      // Track game image loading to dismiss the overlay (non-blocking)
      if (section === 'games' && firstGamesPageLoad) {
        var gameImages = mainContent.querySelectorAll('.game-card img');
        var loadedCount = 0;
        var totalImages = gameImages.length;

        function hideGamesLoader() {
          clearTimeout(window.__gamesLoadTimeout);
          var overlay = document.getElementById('gamesPageLoadingOverlay');
        ...
        function applyFallback(img) {
          ...
          fallback.style.cssText = 'display:flex;align-items:center;justify-content:center;width:100%;height:100%;font-size:2rem;background:rgba(255,255,255,0.05)';
        ...
        if (totalImages === 0) {
          ...
          gameImages.forEach(function(img) {
            if (img.complete && img.naturalWidth > 0) {
              ...
              img.onload = function() {
                loadedCount++;
                if (loadedCount === totalImages) hideGamesLoader();
              };
    ...
    function loadSection(section) {
      ...
      // Close customization menu when navigating away
      const customMenu = document.getElementById('customizationMenu');
      if (customMenu) customMenu.classList.remove('open');
      render(section);
    ...
    async function openGame(id) {
      ...
      if (window._gameLoadTimer) { clearTimeout(window._gameLoadTimer); window._gameLoadTimer = null; }
      window.removeEventListener('resize', window._gameScaleHandler);
      window._gameScaleHandler = null;
      document.body.classList.remove('game-page-active');
      var oldIframe = document.getElementById('gameFrame');
      if (oldIframe) { oldIframe.src = ''; oldIframe.removeAttribute('data-src'); }
      document.querySelectorAll('.is-cluster-rush').forEach(function(e) { e.classList.remove('is-cluster-rush'); });
      ...
      // Per-game scaling (e.g. Cluster Rush Unity 640x480)
      window.removeEventListener('resize', window._gameScaleHandler);
      if (id === 'cluster-rush') {
        var container = document.querySelector('.game-embed-container');
        if (container) container.classList.add('is-cluster-rush');
        window._gameScaleHandler = function() { scaleIframeToViewport(id); };
        window.addEventListener('resize', window._gameScaleHandler);
      }
      ...
      function doLoadGame(gameMode) {
        ...
        // Timer: 30-second content monitor for blank/error/blocked-page detection
        window._gameLoadTimer = setTimeout(function gameLoadMonitor() {
          ...
          try {
            ...
            for (var bi = 0; bi < blockedPatterns.length; bi++) {
              if (txtLower.indexOf(blockedPatterns[bi]) !== -1) {
                console.warn('[GAME-MONITOR] blocked page detected (pattern: "' + blockedPatterns[bi] + '")');
                showBlockedSource(id);
                return;
              }
            }
          } catch(e) {
      ...
      function showBlockedSource(gameId) {
        if (loadingOverlay) {
          loadingOverlay.innerHTML = '<div class="game-loading-error"><svg viewBox="0 0 24 24" width="48" height="48" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg><p>This game source is blocked by security verification. The game needs a new mirror.</p><button onclick="document.getElementById(\'gameLoadingOverlay\').classList.add(\'hidden\');document.getElementById(\'gameLoadingOverlay\').innerHTML=\'\';openGame(\'' + gameId + '\')" class="game-error-retry">Retry</button></div>';
          loadingOverlay.classList.remove('hidden');
        }
      }
    ...
    function openGameTab(id) {
      ...
      if (!id || !gameIndex[id]) {
        ...
        if (iframe && iframe.src && !iframe.src.startsWith('data:')) {
          window.open(iframe.src, '_blank', 'noopener,noreferrer');
        }
        return;
      }
      var baseUrl = window.location.href.split('?')[0].split('#')[0];
      var gameUrl = baseUrl.replace(/\/[^/]*$/, '/game.html') + '?id=' + encodeURIComponent(id);
      window.open(gameUrl, '_blank', 'noopener,noreferrer');
    }

    function scaleIframeToViewport(id) {
      if (id !== 'cluster-rush') return;
      var iframe = document.getElementById('gameFrame');
      if (!iframe) return;
              ...
              ${group.matches.map(item => `
                <button class="home-search-item" data-section="${escapeHTML(group.section)}" data-title="${escapeHTML(item.title)}" onclick="openSearchResult(this.dataset.section, this.dataset.title)">
                  <div class="home-search-icon">
                    ${item.image ? `<img src="${escapeHTML(item.image)}" alt="${escapeHTML(item.title)} icon" onerror="this.onerror=null;this.outerHTML='<span style=display:flex;align-items:center;justify-content:center;width:100%;height:100%;font-size:1.5rem>${escapeHTML(item.emoji)}</span>'">` : item.emoji}
                  </div>
    ...
    function handleSearch(query) {
      if (currentSection && !['settings', 'game', 'proxy'].includes(currentSection)) {
    ...
    function buildCustomizationMenu() {
      const menu = document.getElementById('customizationMenu');
      if (!menu) return;

      var savedBg = localStorage.getItem('orbit-bg') || '';
      var savedParticle = localStorage.getItem('orbit-particle') || 'normal';
      var savedMode = localStorage.getItem('orbit-particle-mode') || 'float-up';

      var bgFiles = [
        { file: 'Black.webp', label: 'Black' },
        { file: 'Green.webp', label: 'Green' },
        { file: 'Orange.jpg', label: 'Orange' },
        { file: 'Purple.webp', label: 'Purple' },
        { file: 'Red.webp', label: 'Red' },
        { file: 'Space.jpg', label: 'Space' },
        { file: 'Minecraft.gif', label: 'Minecraft' },
        { file: 'Zelda.gif', label: 'Zelda' }
      ];

      var densityPresets = [
        { id: 'low', label: 'Low', desc: 'Minimal particles, best performance', icon: '◌' },
        { id: 'normal', label: 'Normal', desc: 'Balanced density and performance', icon: '◍' },
        { id: 'high', label: 'High', desc: 'Full particle effect', icon: '◆' }
      ];

      var motionPresets = [
        { id: 'float-up', label: 'Float Up', desc: 'Particles gently rise upward', icon: '↑' },
        { id: 'float-down', label: 'Float Down', desc: 'Particles gently fall downward', icon: '↓' },
        { id: 'left-to-right', label: 'Left to Right', desc: 'Particles drift right', icon: '→' },
        { id: 'right-to-left', label: 'Right to Left', desc: 'Particles drift left', icon: '←' },
        { id: 'bottom-left-top-right', label: 'Bottom Left → Top Right', desc: 'Diagonal upward right', icon: '↗' },
        { id: 'bottom-right-top-left', label: 'Bottom Right → Top Left', desc: 'Diagonal upward left', icon: '↖' },
        { id: 'top-left-bottom-right', label: 'Top Left → Bottom Right', desc: 'Diagonal downward right', icon: '↘' },
        { id: 'top-right-bottom-left', label: 'Top Right → Bottom Left', desc: 'Diagonal downward left', icon: '↙' },
        { id: 'gentle-wave', label: 'Gentle Wave', desc: 'Smooth wavy floating motion', icon: '∿' },
        { id: 'slow-drift', label: 'Slow Drift', desc: 'Very slow ambient drift', icon: '·' }
      ];

      var activeBgMode = localStorage.getItem('orbit-bg-mode') || 'particle';

      menu.innerHTML =
        '<div class="customization-tabs">' +
          '<div class="customization-pill" id="customPill"></div>' +
          '<button class="customization-tab active" data-panel="image">Image</button>' +
          '<button class="customization-tab" data-panel="particle">Particle</button>' +
        '</div>' +
        '<div class="customization-panel active" id="customPanelImage">' +
          '<div class="customization-label">Background Image</div>' +
          '<div class="bg-card-grid">' +
            '<div class="bg-card' + (savedBg === '' ? ' selected' : '') + '" data-bg="">' +
              '<div style="width:100%;height:100%;background:rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:center;font-size:0.7rem;color:rgba(255,255,255,0.3)">None</div>' +
              '<div class="bg-card-label">None</div>' +
            '</div>' +
            bgFiles.map(function(f) {
              return '<div class="bg-card' + (savedBg === f.file ? ' selected' : '') + '" data-bg="' + f.file + '">' +
                '<img src="backgrounds/' + f.file + '" alt="' + f.label + '" loading="lazy">' +
                '<div class="bg-card-label">' + f.label + '</div>' +
              '</div>';
            }).join('') +
          '</div>' +
        '</div>' +
        '<div class="customization-panel" id="customPanelParticle">' +
          '<div class="customization-label">Particle Density</div>' +
          '<div class="preset-options" id="densityPresets">' +
            densityPresets.map(function(p) {
              return '<div class="preset-option' + (savedParticle === p.id ? ' selected' : '') + '" data-particle="' + p.id + '">' +
                '<div class="preset-option-icon">' + p.icon + '</div>' +
                '<div class="preset-option-info">' +
                  '<div class="preset-option-name">' + p.label + '</div>' +
                  '<div class="preset-option-desc">' + p.desc + '</div>' +
                '</div>' +
              '</div>';
            }).join('') +
          '</div>' +
          '<div class="customization-label" style="margin-top:16px">Particle Motion</div>' +
          '<div class="preset-options" id="motionPresets">' +
            motionPresets.map(function(p) {
              return '<div class="preset-option' + (savedMode === p.id ? ' selected' : '') + '" data-motion="' + p.id + '">' +
                '<div class="preset-option-icon">' + p.icon + '</div>' +
                '<div class="preset-option-info">' +
                  '<div class="preset-option-name">' + p.label + '</div>' +
                  '<div class="preset-option-desc">' + p.desc + '</div>' +
          ...
          '</div>' +
          '<div class="bg-card-grid" style="margin-top:12px;display:none" id="motionPreviewGrid"></div>' +
        '</div>';

      // Tab switching
      var tabs = menu.querySelectorAll('.customization-tab');
      var panels = {
        image: document.getElementById('customPanelImage'),
        particle: document.getElementById('customPanelParticle')
      };
      var pill = document.getElementById('customPill');

      tabs.forEach(function(tab) {
        tab.addEventListener('click', function() {
          tabs.forEach(function(t) { t.classList.remove('active'); });
          this.classList.add('active');
          if (panels.image) panels.image.classList.remove('active');
          if (panels.particle) panels.particle.classList.remove('active');
          var panel = panels[this.dataset.panel];
          if (panel) panel.classList.add('active');
          if (pill) {
            pill.classList.toggle('particle', this.dataset.panel === 'particle');
          }
        });
      });

      // Background selection
      menu.querySelectorAll('.bg-card').forEach(function(card) {
        card.addEventListener('click', function() {
          menu.querySelectorAll('.bg-card').forEach(function(c) { c.classList.remove('selected'); });
      ...
      menu.querySelectorAll('#densityPresets .preset-option').forEach(function(opt) {
        opt.addEventListener('click', function() {
          menu.querySelectorAll('#densityPresets .preset-option').forEach(function(o) { o.classList.remove('selected'); });
          this.classList.add('selected');
          var density = this.dataset.particle;
          localStorage.setItem('orbit-particle', density);
          settings.particleDensity = density;
          settings.particles = density !== 'off';
          saveStoredSettings();
          applySettings();
        });
      });

      // Motion preset selection (no particle recreation)
      menu.querySelectorAll('#motionPresets .preset-option').forEach(function(opt) {
        opt.addEventListener('click', function() {
          menu.querySelectorAll('#motionPresets .preset-option').forEach(function(o) { o.classList.remove('selected'); });
          this.classList.add('selected');
          var mode = this.dataset.motion;
          // Switch to particle mode when selecting motion
          localStorage.setItem('orbit-bg-mode', 'particle');
          document.body.classList.remove('has-custom-bg');
          document.documentElement.style.removeProperty('--custom-bg');
          document.documentElement.style.removeProperty('--custom-bg-size');
          applyParticleMotion(mode);
      });
    }

    function selectBackgroundImage(bg) {
      localStorage.setItem('orbit-bg', bg);
      localStorage.setItem('orbit-bg-mode', 'image');
      var root = document.documentElement;
      if (bg) {
        root.style.setProperty('--custom-bg', 'url(backgrounds/' + bg + ')');
        root.style.setProperty('--custom-bg-size', 'cover');
    ...
    function toggleCustomizationMenu() {
      var menu = document.getElementById('customizationMenu');
      if (!menu) return;
    ...
    // Close customization menu on outside click (delegated, attached once)
    document.addEventListener('click', function(e) {
      var menu = document.getElementById('customizationMenu');
      var editIcon = document.querySelector('.hero-edit-icon');
      if (menu && menu.classList.contains('open')) {
        if (!menu.contains(e.target) && editIcon && !editIcon.contains(e.target)) {
          menu.classList.remove('open');
        }
      }
    });
    ...
    function applyCustomBackground(bg) {
      ...
      if (bg) {
        root.style.setProperty('--custom-bg', 'url(backgrounds/' + bg + ')');
        root.style.setProperty('--custom-bg-size', bg.endsWith('.gif') ? 'cover' : 'cover');
        document.body.classList.add('has-custom-bg');
      } else {
        root.style.removeProperty('--custom-bg');
        root.style.removeProperty('--custom-bg-size');
    ...
    function applyParticlePreset(density) {
      if (settings) {
        settings.particleDensity = density;
        settings.particles = density !== 'off';
        saveStoredSettings();
    ...
    function goHome() {
      var heroWrapper = document.querySelector('.hero-search-wrapper');
      if (heroWrapper) {
        heroWrapper.style.transition = '';
        heroWrapper.style.transform = '';
        heroWrapper.style.opacity = '';
    ...
    function attachHoverSFX() {
      document.querySelectorAll('.game-card, .settings-reset, .settings-nav-item, .home-search-item, .suggestion-card, .icon-btn, .proxy-open-btn').forEach(el => {
        if (!el.dataset.sfx) {
          el.dataset.sfx = "1";
    ...
    function startOnboarding() {
      ...
      if (onboardingFlow && introScreen) {
        ...
        // Preload theme images while user is on steps 1-2
        Object.values(themeIconUrls).forEach(function(url) {
          var img = new Image();
          img.src = url;
        });
    ...
    function initializeIntro() {
      ...
      if (checkOnboarding()) {
        ...
        // Returning user: reveal hero section, dock, edit icon
        if (introScreen) {
          introScreen.style.display = 'none';
        }
        initHomepage();
      ...
      themes.forEach(themeName => {
        ...
        card.innerHTML = `
          <div class="onboarding-theme-graphic">
            <img src="${escapeHTML(iconUrl)}" alt="${escapeHTML(themeName.charAt(0).toUpperCase() + themeName.slice(1))}" class="onboarding-theme-icon">
          </div>
          <div class="onboarding-theme-name">${escapeHTML(themeName.charAt(0).toUpperCase() + themeName.slice(1))}</div>
        `;
    ...
    function initHeroClock() {
      ...
      clockEl.innerHTML = `
        <div class="hero-search-wrapper">
          ...
          <input type="text" class="hero-search-input" placeholder="Search the web..." />
        </div>
      `;

      const input = clockEl.querySelector('.hero-search-input');
      if (!input) return;
      ...
      input.addEventListener('keydown', function (e) {
        ...
        const wrapper = clockEl.querySelector('.hero-search-wrapper');
        if (wrapper) {
          wrapper.style.transition = 'transform 0.15s ease, opacity 0.15s ease';
          ...
          requestAnimationFrame(function () {
            requestAnimationFrame(function () {
              if (window.VoltraBrowser && window.VoltraBrowser.navigate) {
                window.VoltraBrowser.navigate(value);
              }
            });
          });
        }, 120);
    ...
    // Material-style SVG icons for each notification type
    const notifTypeSvgs = {
      update: '<svg viewBox="0 -960 960 960" fill="currentColor"><path d="m480-80-74-214-212-74 212-74 74-212 74 212 212 74-212 74-74 214Z"/></svg>',
      tip: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="6" width="20" height="13" rx="2"/><line x1="5.5" y1="9.5" x2="8.5" y2="9.5"/><line x1="11" y1="9.5" x2="15" y2="9.5"/><line x1="17" y1="9.5" x2="18.5" y2="9.5"/><line x1="5" y1="12.5" x2="9.5" y2="12.5"/><line x1="11" y1="12.5" x2="15" y2="12.5"/><line x1="17" y1="12.5" x2="19" y2="12.5"/><line x1="7" y1="15.5" x2="17" y2="15.5"/></svg>',
      game: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="9" width="18" height="6" rx="2"/><line x1="8" y1="11" x2="8" y2="13"/><line x1="7" y1="12" x2="9" y2="12"/><circle cx="16" cy="11" r="1"/><circle cx="16" cy="13" r="1"/></svg>',
      announce: '<svg viewBox="0 -960 960 960" fill="currentColor"><path d="M480-80q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm-40-82v-78q-33 0-56.5-23.5T360-320v-40L168-552q-3 18-5.5 36t-2.5 36q0 121 79.5 212T440-162Zm276-102q20-22 36-47.5t26.5-53q10.5-27.5 16-56.5t5.5-59q0-98-54.5-179T600-776v16q0 33-23.5 56.5T520-680h-80v80q0 17-11.5 28.5T400-560h-80v80h240q17 0 28.5 11.5T600-440v120h40q26 0 47 15.5t29 40.5Z"/></svg>',
    };

    const defaultNotifications = [
      { id: 'n1', type: 'update',   title: 'Orbit v2.1 Released',              description: 'Ultraviolet proxy, 50+ games, and a redesigned glass interface.' },
      { id: 'n2', type: 'announce', title: 'Ultraviolet proxy ready',           description: 'Browse freely with service worker support and bare server routing.' },
      { id: 'n3', type: 'game',     title: 'Game library refreshed',            description: 'Cleaner self-hosted sources, source health checking, and broken-source detection.' },
      { id: 'n4', type: 'tip',      title: 'Customize your Orbit',              description: 'Particle backgrounds, glowing orbs, background music, and theme controls.' },
      { id: 'n5', type: 'announce', title: 'Search & browsing tools',           description: 'Homepage search, section search, and streamlined address bar navigation.' },
      { id: 'n6', type: 'tip',      title: 'Visual atmosphere controls',         description: 'Adjust particle density, orb visibility, glow, and layout density.' },
      { id: 'n7', type: 'game',     title: '50+ games to play',                 description: 'From Minecraft and Mario Kart to FNAF, Slope, and Basketball Stars.' },
      { id: 'n8', type: 'update',   title: 'Help & support system',             description: 'In-app FAQ, quick start guide, and troubleshooting for proxy and games.' },
    ];

    function buildNotifSlideContent(notif) {
      const iconSvg = notifTypeSvgs[notif.type] || notifTypeSvgs.announce;
      return '<div class="notif-card-inner">' +
    ...
    function buildPaginationDots(count, activeIndex) {
      var html = '';
      for (var i = 0; i < count; i++) {
        html += '<span class="notif-dot' + (i === activeIndex ? ' active' : '') + '" data-index="' + i + '"></span>';
      }
      return html;
    }
    ...
    function renderNotifications() {
      ...
      var pageCount = Math.min(notifications.length, NOTIFS_PER_PAGE);
      var slidesHtml = '';
      for (var i = 0; i < pageCount; i++) {
        slidesHtml += '<div class="notif-slide-item">' + buildNotifSlideContent(notifications[i]) + '</div>';
      }
      var dotsHtml = buildPaginationDots(pageCount, notifCurrentPage);

      var featuredHtml = '<div class="notif-card featured">' +
        '<div class="notif-card-inner">' +
          '<div class="notif-slide-wrap">' +
            '<div class="notif-slide-track" style="transform:translateX(-' + (notifCurrentPage * 100) + '%)">' +
              slidesHtml +
            '</div>' +
          '</div>' +
          '<div class="notif-expand-btn' + (notifExpanded ? ' expanded' : '') + '">' +
            '<svg viewBox="0 -960 960 960" fill="currentColor"><path d="M480-345 240-585l56-56 184 184 184-184 56 56-240 240Z"/></svg>' +
          '</div>' +
        '</div>' +
        '<div class="notif-pagination' + (notifExpanded ? ' fade-out' : '') + '">' + dotsHtml + '</div>' +
      '</div>';

      var remainingHtml = '';
      for (var r = NOTIFS_PER_PAGE; r < notifications.length; r++) {
        var n = notifications[r];
        remainingHtml += '<div class="notif-card collapsed" data-index="' + r + '">' +
          buildNotifSlideContent(n) +
        '</div>';
      }
      ...
      return featuredHtml + remainingHtml;
    }

    function applyNotificationState(el, animate) {
      if (!el) return;

      var cards = el.querySelectorAll('.notif-card:not(.featured)');
      var pagination = el.querySelector('.notif-pagination');
      var expandBtn = el.querySelector('.notif-expand-btn');
      ...
      if (notifExpanded) {
        cards.forEach(function(card, i) {
          ...
          card.style.maxHeight = (card.scrollHeight + 30) + 'px';
        });
        if (pagination) pagination.classList.add('fade-out');
        if (expandBtn) expandBtn.classList.add('expanded');
      } else {
        cards.forEach(function(card, i) {
          if (animate) {
            card.style.transitionDelay = ((cards.length - 1 - i) * 60) + 'ms';
          } else {
            card.style.transitionDelay = '0ms';
          }
        ...
        if (pagination) pagination.classList.remove('fade-out');
        if (expandBtn) expandBtn.classList.remove('expanded');
      }
    }

    function positionNotificationHub() {
      var el = document.getElementById('heroNotifications');
    ...
    function initNotifications() {
      var el = document.getElementById('heroNotifications');
      if (!el) return;
      if (notifications.length === 0) notifications = defaultNotifications.map(function(n) { return Object.assign({}, n); });
      notifExpanded = false;
      notifCurrentPage = 0;
    ...
    function dismissNotification(id) {
      ...
      var el = document.getElementById('heroNotifications');
      if (!el) return;
      if (notifCurrentPage >= Math.min(notifications.length, NOTIFS_PER_PAGE)) {
        notifCurrentPage = Math.max(0, Math.min(notifications.length, NOTIFS_PER_PAGE) - 1);
      }
      el.innerHTML = renderNotifications();
      applyNotificationState(el, false);
    }
    ...
    function addNotification(notif) {
      ...
      if (notifications.some(function(n) { return n.id === notif.id; })) return null;
    ...
    document.addEventListener('click', function(e) {
      var dot = e.target.closest('.notif-dot');
      if (dot) {
        var index = parseInt(dot.getAttribute('data-index'), 10);
        if (!isNaN(index) && index !== notifCurrentPage) {
          notifCurrentPage = index;
          var el = document.getElementById('heroNotifications');
          if (!el) return;
          var track = el.querySelector('.notif-slide-track');
          if (track) {
            track.style.transform = 'translateX(-' + (index * 100) + '%)';
          }
          el.querySelectorAll('.notif-dot').forEach(function(d) { d.classList.remove('active'); });
          dot.classList.add('active');
        }
        return;
      }

      var btn = e.target.closest('.notif-expand-btn');
      if (!btn) return;
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Auto-open game from URL param (used by game.html)
      if (window.__ORBIT_AUTO_GAME_ID__ && gameIndex[window.__ORBIT_AUTO_GAME_ID__]) {
        requestAnimationFrame(function() {
          openGame(window.__ORBIT_AUTO_GAME_ID__);
        });
      }
    });
