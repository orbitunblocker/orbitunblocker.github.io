...
async function main() {
  ...
  const chrome = spawn(
    ...
    [
      ...
      `--user-data-dir=C:\\Users\\abeni\\AppData\\Local\\Temp\\cdbg-${Date.now()}`,
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (i % 5 === 0) console.log('Waiting for proxy boot...', i);
  }
  console.log('Proxy booted');

  // List all iframes
  const r1 = await send('Runtime.evaluate', {
    expression: 'Array.from(document.querySelectorAll("iframe")).map(f=>f.id+"="+(f.src||"").substring(0,100))',
    returnByValue: true
  });
  console.log('Iframes:', JSON.stringify(r1.result?.result?.value));

  // Check navigate function
  const r2 = await send('Runtime.evaluate', {
    expression: 'typeof VoltraBrowser._browserUI.navigate',
    returnByValue: true
  });
  console.log('navigate type:', r2.result?.result?.value);
  ...
  const r3 = await send('Runtime.evaluate', {
    expression: 'Array.from(document.querySelectorAll("iframe")).map(f=>f.id+"="+(f.src||"").substring(0,120))',
    returnByValue: true
  });
  console.log('After nav:', JSON.stringify(r3.result?.result?.value));
  ...
  for (let s = 1; s <= 10; s++) {
    ...
    console.log(`T+${s+2}:`, JSON.stringify(r.result?.result?.value));
  }
