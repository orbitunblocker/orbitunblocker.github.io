...
async function main() {
  ...
  const chrome = spawn(
    ...
    [
      ...
      `--user-data-dir=C:\\Users\\abeni\\AppData\\Local\\Temp\\cdbg2-${Date.now()}`,
  ...
  for (let i = 0; i < 30; i++) {
    await sleep(1000);
    try { await reqJSON(`http://127.0.0.1:${CDP_PORT}/json/version`); break; } catch (e) {}
  }
  ...
  // Check page URL, body HTML (first 500 chars), and iframe count
  const info = await send('Runtime.evaluate', {
    expression: 'JSON.stringify({url:location.href,html:document.body?document.body.innerHTML.substring(0,300):"nobody",iframes:document.querySelectorAll("iframe").length,title:document.title})',
    returnByValue: true
  });
  console.log('Before nav:', info.result?.result?.value);

  // Navigate
  const navResult = await send('Runtime.evaluate', {
    expression: '(function(){try{VoltraBrowser._browserUI.navigate("https://www.cnn.com");return "OK"}catch(e){return "ERR: "+e.message}})()',
    returnByValue: true
  });
  ...
  const info2 = await send('Runtime.evaluate', {
    expression: 'JSON.stringify({url:location.href,html:document.body?document.body.innerHTML.substring(0,500):"nobody",iframes:document.querySelectorAll("iframe").length,title:document.title})',
    returnByValue: true
  });
  console.log('After nav:', info2.result?.result?.value);
