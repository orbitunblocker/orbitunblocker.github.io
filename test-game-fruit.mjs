...
async function main() {
  ...
  const chrome = spawn(CHROME, [
    ...
    `--user-data-dir=C:\\Users\\abeni\\AppData\\Local\\Temp\\gtrace3-${Date.now()}`,
    TARGET + '/'
  ]);
  ...
  for (let sec = 1; sec <= 20; sec++) {
    ...
    const r = await send('Runtime.evaluate', {
      expression: `(function(){var f=document.getElementById('gameFrame');if(!f)return 'NOFRAME';try{var d=f.contentDocument;if(!d||!d.body)return JSON.stringify({html:'NODOC',len:0});var html=d.documentElement?d.documentElement.outerHTML:'';return JSON.stringify({html:html.substring(0,200),len:html.length})}catch(e){return JSON.stringify({html:'CROSS:'+e.message,len:-1})}})()`,
      returnByValue: true
    });
    const val = r.result?.result?.value;
    try {
      const parsed = JSON.parse(val);
      console.log(`T+${sec}: htmlLen=${parsed.len} ${parsed.html.substring(0,100)}`);
    } catch(e) {
  ...
  console.log(`\nRequests: ${networkReqs.length}, Failed: ${failedReqs.length}, Logs: ${afterLaunch.length}, Exceptions: ${exceptions.length}`);
